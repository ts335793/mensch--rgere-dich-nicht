...
mainLoop g prev t = do
    ...
    case c of
        ...
        GenerateMove p d -> do
            ...
            --(src, dst) <- monteCarloTreeSearch prefereBeatPlayout g2 3000 -- does not work against pawlewicz
            --((src, dst), sts) <- nestedMonteCarlo prefereBeatPlayout g2 5 15 -- mein hero
            --((src, dst), sts) <- nestedMonteCarlo firstForwardPlayout g2 5 15 -- not so good
            -- ucb + aff - poor shit
            --((src, dst), sts) <- monteCarlo firstForwardPlayout g2 500 -- not too good
            ((src, dst), sts) <- nestedMonteCarlo (antiFirstForwardPlayout p) g2 5 15
...
{-((src, dst), sts) <- monteCarlo pdp g2 1000
((src, dst), sts) <- monteCarlo playout g2 1000
((src, dst), sts) <- monteCarloUCB pdp g2 1000
((src, dst), sts) <- monteCarloUCB playout g2 1000
((src, dst), sts) <- nestedMonteCarlo (monteCarlo pdp) g2 3 1000
((src, dst), sts) <- nestedMonteCarlo (monteCarlo playout) g2 3 1000
((src, dst), sts) <- nestedMonteCarlo (monteCarloUCB pdp) g2 3 1000
((src, dst), sts) <- nestedMonteCarlo (monteCarloUCB playout) g2 3 1000
(src, dst) <- monteCarloTreeSearch pdp g2 6000
(src, dst) <- monteCarloTreeSearch playout g2 6000-}
--(src, dst) <- monteCarloTreeSearch randomPlayout g2 1000
--(src, dst) <- monteCarloTreeSearch (antiFirstForwardPlayout p) g2 1000
--((src, dst), sts) <- monteCarloUCB (antiFirstForwardPlayout p) g2 1000
--(src, dst) <- monteCarloTreeSearch (antiFirstForwardPlayout p) g2 2000